...
def validate_cookie(cookie):
    ...
    for part in cookie.split(";"):
        ...
        cookie_parts[name.strip()] = value.strip()

    # A lone c_user value is not a usable browser session.
    return bool(cookie_parts.get("c_user") and cookie_parts.get("xs"))
...
async def get_token_from_cookie(cookie, session):
    ...
    try:
        ...
        async with session.get('https://business.facebook.com/content_management', headers=head) as response:
            ...
            if not token_match:
                st.error("Could not find an access token. The cookie may be incomplete, expired, or blocked by Facebook verification.")
                return None
